...
static void count_unidir_routing_transistors(segment_info_t* segment_inf,
                                             double R_minW_nmos,
                                             double R_minW_pmos);

static double get_cblock_trans(int* num_inputs_to_cblock,
...
static double trans_per_buf(double Rbuf,
                           double R_minW_nmos,
                           double R_minW_pmos);

static double trans_per_mux(int num_inputs,
                           double trans_sram_bit,
                           double pass_trans_area);

static double trans_per_R(double Rtrans,
                         double R_minW_trans);
...
void
count_routing_transistors(directionality_t directionality,
                          int num_switch,
...
void
count_unidir_routing_transistors(segment_info_t* segment_inf,
                                 double R_minW_nmos,
                                 double R_minW_pmos)
{
    boolean* cblock_counted;    /* [0..max(num_grid_columns,num_grid_rows)] -- 0th element unused. */
    ...
    printf("Total Routing Area: %#g  Per logic tile: %#g\n", ntrans,
           ntrans / (double)(num_grid_columns * num_grid_rows));
}
              ...
              double pass_trans_area)
{
     ...
     * levels.                                                                  */
    double ntrans, sram_trans, pass_trans;
    ...
    if (num_inputs <= 1) {
        ...
        sram_trans =
            (ceil((double)num_inputs / num_second_stage_trans - 0.00001) +
