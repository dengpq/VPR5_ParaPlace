...
static pic_type_t pic_on_screen = NO_PICTURE;    /* What do I draw? */

static double* tile_x, *tile_y;

/* The left and bottom coordinates of each grid_tile in the FPGA.         *
 * tile_x[0..num_grid_columns+1] and tile_y[0..num_grid_rows+1].                         *
 * COORDINATE SYSTEM goes from (0,0) at the lower left corner to          *
 * (tile_x[num_grid_columns+1]+tile_width, tile_y[num_grid_rows+1]+tile_width) in the      *
 * upper right corner.                                                    */


static double tile_width, pin_size;
...
static void highlight_blocks(double x,
                             double y);
static void get_block_center(int block_num,
                             double* x,
                             double* y);
...
static int get_track_num(int inode,
                         int** chanx_track,
                         int** chany_track);
static void draw_rr_switch(double from_x,
                           double from_y,
                           double to_x,
                           double to_y,
                           boolean buffered);
static void draw_triangle_along_line(double xend,
                                     double yend,    /* UDSD by AY */

                                     double x1,
                                     double x2,  /* UDSD by AY */

                                     double y1,
                                     double y2); /* UDSD by AY */
...
alloc_draw_structs(void)
{
    ...
    tile_x = (double*)my_malloc((num_grid_columns + 2) * sizeof(double));
    tile_y = (double*)my_malloc((num_grid_rows + 2) * sizeof(double));
...
void
init_draw_coords(double width_val)
{
    ...
    tile_x[num_grid_columns + 1] = ((num_grid_columns + 1) * tile_width) + j;
    j = 0;
    ...
    tile_y[num_grid_rows + 1] = ((num_grid_rows + 1) * tile_width) + j;
    init_world(0.0,
               tile_y[num_grid_rows + 1] + tile_width, tile_x[num_grid_columns + 1] + tile_width, 0.0);
}
...
static void drawplace(void)
{
    ...
    for (i = 0; i <= (num_grid_columns + 1); i++) {
        for (j = 0; j <= (num_grid_rows + 1); j++) {
            ...
            /* Don't draw corners */
            if (((i < 1) || (i > num_grid_columns)) && ((j < 1) || (j > num_grid_rows))) {
                continue;
            }
            ...
            for (k = 0; k < num_sub_tiles; ++k) {
                ...
                if ((i < 1) || (i > num_grid_columns)) {
                    /* left and right fringes */
                    x1 = tile_x[i];
                    y1 = tile_y[j] + (k * sub_tile_step);
                    x2 = x1 + tile_width;
                    y2 = y1 + sub_tile_step;
                } else if ((j < 1) || (j > num_grid_rows)) {
                ...
                setcolor(BLACK);
                setlinestyle((EMPTY == block_num) ? DASHED : SOLID);
                drawrect(x1, y1, x2, y2);

                /* Draw text if the space has parts of the netlist */
                if (block_num != EMPTY) {
                    drawtext((x1 + x2) / 2.0, (y1 + y2) / 2.0,
                             blocks[block_num].name, tile_width);
                }
...
static void drawnets(void)
{
    ...
    for (inet = 0; inet < num_nets; ++inet) {
        ...
        b1 = net[inet].node_blocks[0];   /* The DRIVER */
...
static void
get_block_center(int block_num,
                 double* x,
                 double* y)
{
    ...
    int k = blocks[block_num].z;
    double sub_tile_step = tile_width / blocks[block_num].block_type->capacity;
              ...
              int itrack)
{
    ...
    { BUFFSIZE = 80 };
    double x1, x2, y;
    double y1, y2;       /* UDSD by AY */
    int k;          /* UDSD by AY */
    char str[BUFFSIZE];
    /* Track 0 at bottom tedge, closest to "owning" clb. */
    ...
    if (rr_node[inode].direction == INC_DIRECTION) {
        ...
        /* TODO: this looks odd, why does it ignore final blocks? does this mean nothing appears with L=1 ? */
              ...
              int itrack)
{
    ...
    { BUFFSIZE = 80 };
    double x, y1, y2;
    double x1, x2;       /* UDSD by AY */
    int k;          /* UDSD by AY */
    char str[BUFFSIZE];
    /* Track 0 at left tedge, closest to "owning" clb. */
...
draw_chanx_to_chany_edge(int chanx_node,
                         ...
                         short switch_type)
{
    /* Draws an tedge (SBOX connection) between an x-directed channel and a    *
     * y-directed channel.                                                    */
    double x1, y1, x2, y2;
    ...
    /* UDSD Modification by WMF Begin */
        ...
        if (rr_node[to_node].direction != BI_DIRECTION) {
            ...
            if (to_xlow < from_xlow) {
                /* Draw from left tedge of one to other */
                x1 = tile_x[from_xlow];
                ...
                /* Draw from right tedge of one to other */
                x1 = tile_x[from_xhigh] + tile_width;
                         ...
                         short switch_type)
{
    ...
    /* UDSD Modification by WMF Begin */
        ...
        if (rr_node[to_node].direction != BI_DIRECTION) {
            if (to_track % 2 == 0) {
                /* INC wire starts at bottom tedge */
                assert(from_ylow < to_ylow);
                y2 = tile_y[to_ylow];
                /* since no U-turns from_track must be INC as well */
                y1 = tile_y[to_ylow - 1] + tile_width;
            } else {
                /* DEC wire starts at top tedge */
            ...
            if (to_ylow < from_ylow) {
                /* Draw from bottom tedge of one to other. */
                y1 = tile_y[from_ylow];
                ...
                /* Draw from top tedge of one to other. */
                y1 = tile_y[from_yhigh] + tile_width;
...
static void
draw_rr_switch(double from_x,
               double from_y,
               double to_x,
               double to_y,
               boolean buffered)
{
    /* Draws a buffer (triangle) or pass transistor (circle) on the tedge        *
    ...
    if (!buffered) {
        ...
        /* Recall: perpendicular vector_t to the unit vector_t along the switch (xv, yv) *
         * is (yv, -xv).                                                             */
...
drawroute(enum e_draw_net_type draw_net_type)
{
    ...
    short switch_type;
    trace_t* tptr;
    rr_type_t rr_type, prev_type;

    if (draw_route_type == GLOBAL) {
        /* Allocate some temporary storage if it's not already available. */
        if (chanx_track == NULL) {
            chanx_track =
                (int**)alloc_matrix(1, num_grid_columns, 0, num_grid_rows, sizeof(int));
        }

        if (chany_track == NULL) {
            chany_track =
                (int**)alloc_matrix(0, num_grid_columns, 1, num_grid_rows, sizeof(int));
        }
                             ...
                             double y)
{
    ...
    for (i = 0; i <= (num_grid_columns + 1) && !hit; i++) {
    ...
    if ((i < 1) || (i > num_grid_columns)) { /* Vertical columns of IOs */
    ...
    for (k = 0; k < type->num_type_pins; k++) {
        ...
        if (type->class_inf[iclass].type == DRIVER) {
            ...
            /* This net is fanin to the blocks. */
            net_color[netnum] = BLUE;
            fanblk = net[netnum].node_blocks[0]; /* DRIVER to net */
            block_color[fanblk] = BLUE;
        }
    }

    block_color[block_num] = GREEN;  /* Selected blocks. */
    sprintf(msg, "blocks %d (%s) at (%d, %d) selected.", block_num,
            blocks[block_num].name, i, j);
    update_message(msg);
...
deselect_all(void)
{
    ...
    for (i = 0; i < num_blocks; i++) {
        if (blocks[i].block_type->index < 3) {
            block_color[i] = LIGHTGREY;
        } else if (blocks[i].block_type->index < 3 + MAX_BLOCK_COLOURS) {
            block_color[i] = BISQUE + MAX_BLOCK_COLOURS + blocks[i].block_type->index - 3;
        } else {
...
/* UDSD by AY Start */
static void
draw_triangle_along_line(double xend,
                         double yend,
                         double x1,
                         double x2,
                         double y1,
                         double y2)
{
    double switch_rad = 0.15;
    double xdelta, ydelta;
    double magnitude;
    double xunit, yunit;
    double xbaseline, ybaseline;
    t_point poly[3];
    xdelta = x2 - x1;
    ydelta = y2 - y1;
    magnitude = sqrt(xdelta * xdelta + ydelta * ydelta);
...
draw_pin_to_chan_edge(int pin_node,
                      int chan_node)
{
    /* This routine draws an tedge from the pin_node to the chan_node (CHANX or   *
    ...
    double draw_pin_off;
    side_types_t iside;
    enum e_direction direction = rr_node[chan_node].direction;
    int pin_num = rr_node[pin_node].ptc_num;
    rr_type_t chan_type = rr_node[chan_node].type;
    int itrack = rr_node[chan_node].ptc_num;
