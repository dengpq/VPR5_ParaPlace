...
static void directed_search_expand_trace_segment(trace_t* start_ptr,
                                                 int target_node,
                                                 double astar_fac,
                                                 int
...
static double get_directed_search_expected_cost(int inode,
                                               int target_node);
...
boolean
try_directed_search_route(router_opts_t router_opts,
                          ...
                          t_mst_edge** mst)
{
    ...
    /* Usually the first iter uses a very small (or 0) pres_fac to find  *
     * the shortest path and get a congestion map.  For fast compiles, I set  *
     * pres_fac high even for the first iter.                            */
    ...
    for (itry = 1; itry <= router_opts.max_router_iterations; itry++) {
        ...
        if (success) {
            ...
            sprintf(msg,
                    "After iter %d routing failed (A*) with a channel width factor of %d and Fc_int of %d, Fs_int of %d.",
        ...
        if (itry == 1) {
            ...
            pathfinder_update_cost(pres_fac, 0.);   /* Acc_fac=0 for first inner_iter_num. */
        } else {
                          ...
                          t_mst_edge** mst)
{
    ...
    /* WMF: For the 1st router iter trace_head[inet] is NULL, as it is
    ...
    for (itarget = 0; itarget < net[inet].num_sinks; itarget++) {
         ...
         * Note: in the 1st iter, there is no trace (no routing at all for this net)
...
static void
directed_search_expand_trace_segment(trace_t* start_ptr,
                                     int target_node,
                                     double astar_fac,
                                  ...
                                  double astar_fac)
{
    ...
    double new_tot_cost, old_back_pcost, new_back_pcost;
    inode = current->index;
