...
void
add_rr_graph_C_from_switches(double C_ipin_cblock)
{
        ...
        if (from_rr_type == CHANX || from_rr_type == CHANY) {
            for (iedge = 0; iedge < rr_node[inode].num_edges; iedge++) {
                ...
                if (to_rr_type == CHANX || to_rr_type == CHANY) {
                     ...
                     * a pass transistor tedge from x to y, I always build the graph *
                     * so that there is a corresponding tedge using the same switch  *
                     ...
                     * If an tedge uses a buffer I always have to add in the output  *
                    ...
                    if (!buffered && inode < to_node) {
                        ...
                        if (rr_node[to_node].drivers !=
                             ...
                             * be added now since each tedge is actually a driver */
                            rr_node[to_node].C +=
                                Cout;
                        }
                ...
                /* End tedge to CHANX or CHANY node. */
    ...
    Couts_to_add = (double*)my_calloc(num_rr_nodes, sizeof(double));
