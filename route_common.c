...
static trace_t* trace_free_head = NULL;
...
static linked_double_ptr* rr_modified_head = NULL;
static linked_double_ptr* linked_f_pointer_free_head = NULL;
...
static trace_t* alloc_trace_data(void);
static void add_to_heap(struct s_heap* hptr);
static struct s_heap* alloc_heap_data(void);
static linked_double_ptr* alloc_linked_f_pointer(void);
...
void
save_routing(trace_t** best_routing,
             ...
             vector_t** saved_clb_opins_used_locally)
{
     ...
     * that the next iter of the router does not free the saved        *
...
void
restore_routing(trace_t** best_routing,
                vector_t** fb_opins_used_locally,
                vector_t** saved_clb_opins_used_locally)
{
...
get_serial_num(void)
{
    ...
    for (inet = 0; inet < num_nets; inet++) {
        ...
        while (tptr != NULL) {
            inode = tptr->index;
            serial_num +=
                (inet + 1) * (rr_node[inode].xlow * (num_grid_columns + 1) -
                              rr_node[inode].yhigh);
...
void
pathfinder_update_one_cost(trace_t* route_segment_start,
                           int add_or_sub,
                           double pres_fac)
{
...
void
pathfinder_update_cost(double pres_fac,
                       double acc_fac)
{
     ...
     * signals sharing a resource right now (i.e. after each complete iter) *
...
update_traceback(struct s_heap* hptr,
    ...
    trace_t* tptr, *prevptr, *temptail, *ret_ptr;
    int inode;
    short iedge;
             ...
             double R_upstream)
{
     ...
     * is stored to make traceback easy.  The index of the tedge used to get     *
...
vector_t**
alloc_route_structs(subblock_data_t subblock_data)
{
    /* Allocates the data structures needed for routing.    */
    vector_t** fb_opins_used_locally;
    trace_head = (trace_t**)my_calloc(num_nets,
                                             sizeof(trace_t*));
    trace_tail = (trace_t**)my_malloc(num_nets *
                                             sizeof(trace_t*));
...
trace_t**
alloc_saved_routing(vector_t** fb_opins_used_locally,
    ...
    trace_t** best_routing;
    vector_t** saved_clb_opins_used_locally;
    int iblk, iclass, num_local_opins;
    block_type_ptr type;

    best_routing = (trace_t**)my_calloc(num_nets,
                                               sizeof(trace_t*));
...
static vector_t**
alloc_and_load_clb_opins_used_locally(subblock_data_t subblock_data)
{
...
void
free_route_structs(vector_t** fb_opins_used_locally)
{
...
void
free_saved_routing(trace_t** best_routing,
                   vector_t** saved_clb_opins_used_locally)
{
...
alloc_trace_data(void) {
    ...
    if (trace_free_head == NULL) {
        /* No elements on the free list */
        trace_free_head = (trace_t*)my_malloc(NCHUNK *
                                                     sizeof(struct
                                                            s_trace));
...
alloc_linked_f_pointer(void) {
    ...
    int i;
    linked_double_ptr* temp_ptr;

    if (linked_f_pointer_free_head == NULL) {
        /* No elements on the free list */
        linked_f_pointer_free_head = (linked_double_ptr*)
                                     my_malloc(NCHUNK * sizeof(linked_double_ptr));
...
void
reserve_locally_used_opins(double pres_fac,
