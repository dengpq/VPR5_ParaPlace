...
static double get_delay_normalization_fac(int nodes_per_chan,
                                         vector_t** * rr_node_indices);

static double get_average_opin_delay(vector_t** * rr_node_indices,
                                    int nodes_per_chan);
 ...
 * own cost_index.                                                           */
void
alloc_and_load_rr_indexed_data(IN segment_info_t* segment_inf,
                               IN int num_segment,
...
get_delay_normalization_fac(int nodes_per_chan,
                            vector_t** * rr_node_indices)
{
    /* Returns the average Tdel to go 1 FB distance along a wire.  */
    ...
    for (itrack = 0; itrack < nodes_per_chan; itrack++) {
        inode =
            get_rr_node_index((num_grid_columns + 1) / 2, (num_grid_rows + 1) / 2, CHANX, itrack,
                              rr_node_indices);
    ...
    for (itrack = 0; itrack < nodes_per_chan; itrack++) {
        inode =
            get_rr_node_index((num_grid_columns + 1) / 2, (num_grid_rows + 1) / 2, CHANY, itrack,
                              rr_node_indices);
        cost_index = rr_node[inode].cost_index;
        frac_num_seg = clb_dist * rr_indexed_data[cost_index].inv_length;
        Tdel = frac_num_seg * rr_indexed_data[cost_index].T_linear +
...
static double
get_average_opin_delay(vector_t** * rr_node_indices,
                       int nodes_per_chan)
{
     ...
     * the average Tdel of non-IO blocks */
                              ...
                              segment_info_t* segment_inf)
{
    ...
    for (itrack = 0; itrack < nodes_per_chan; itrack++) {
        inode =
            get_rr_node_index((num_grid_columns + 1) / 2, (num_grid_rows + 1) / 2, rr_type, itrack,
                              rr_node_indices);
        cost_index = rr_node[inode].cost_index;
