...
static void
map_button(int block_num)
{
    /* Maps a button onto the screen and set it up for input, etc.        */
    button[block_num].win = XCreateSimpleWindow(display, menu,
                                           button[block_num].xleft,
                                           button[block_num].ytop,
                                           button[block_num].width,
                                           button[block_num].height, 0,
              ...
              void (*button_func)(void (*drawscreen)(void)))
{
    ...
    my_strncpy(button[block_num].text, button_text, BUTTON_TEXT_LEN);
...
drawbut(int block_num)
{
    ...
    if (button[block_num].ispoly) {
        ...
        XSetForeground(display, gc_menus, colors[BLACK]);
        XFillPolygon(display, button[block_num].win, gc_menus, mypoly, 3,
...
void
event_loop(void (*act_on_button)(double x,
                                 double y),
