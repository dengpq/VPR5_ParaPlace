...
static void view_mux_size_distribution(vector_t** * rr_node_indices,
                                       int nodes_per_chan,
                                        ...
                                        OUT boolean* Fc_clipped)
{
    ...
    /* double spacing, step_size, f_mux; */
...
static void
view_mux_size_distribution(vector_t** * rr_node_indices,
                           int nodes_per_chan,
                           segment_details_t* seg_details_x,
                           segment_details_t* seg_details_y)
{
    ...
    double* percent_range_array;
    double percent_range, percent_range_sum, avg_percent_range;
    double std_dev_percent_range, deviation_f;
    int range, *range_array, global_max_range;
    double avg_range, range_sum, std_dev_range;
    ...
    percent_range_array =
        (double*)my_malloc((num_grid_columns - 1) * (num_grid_rows - 1) * sizeof(double));
    range_array = (int*)my_malloc((num_grid_columns - 1) * (num_grid_rows - 1) * sizeof(int));
    array_count = 0;
    ...
    for (j = (num_grid_rows - 1); j > 0; j--) {
        for (i = 1; i < num_grid_columns; i++) {
            ...
            percent_range = ((double)range) / ((double)min_value);
    ...
    avg_percent_range = (double)percent_range_sum / array_count;
    avg_range = (double)range_sum / array_count;
    percent_range_sum = 0.0;
    range_sum = 0.0;
    ...
    for (k = 0; k < array_count; k++) {
        ...
        deviation_f = ((double)range_array[k] - avg_range);
        range_sum += deviation_f * deviation_f;
    }

    std_dev_percent_range =
        sqrt(percent_range_sum / ((double)array_count - 1.0));
    std_dev_range = sqrt(range_sum / ((double)array_count - 1.0));
