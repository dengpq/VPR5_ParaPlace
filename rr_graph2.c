...
static void load_chan_rr_indices(IN int nodes_per_chan,
                                 ...
                                 INOUT int* index,
                                 INOUT vector_t** * indices);

static int get_bidir_track_to_chan_seg(IN vector_t conn_tracks,
...
static int get_unidir_track_to_chan_seg(IN boolean is_end_sb,
                                        ...
                                        IN side_types_t from_side,
                                        IN side_types_t to_side,
                                        IN int Fs_per_side,
                           ...
                           IN directionality_t directionality)
{
    ...
    *nodes_per_chan = tmp;
    seg_details = (segment_details_t*)
                  my_malloc(*nodes_per_chan * sizeof(segment_details_t));
...
void
free_seg_details(segment_details_t* seg_details,
                 int nodes_per_chan)
{
 ...
 * for debugging.                                                           */
void
dump_seg_details(segment_details_t* seg_details,
                 int nodes_per_chan,
                 char* fname)
{
    FILE* fp;
 ...
 * started.                                                                  */
int
get_seg_start(IN segment_details_t* seg_details,
              IN int itrack,
              IN int chan_num,
              IN int seg_num)
{
...
int
get_seg_end(IN segment_details_t* seg_details,
            IN int itrack,
            IN int istart,
            IN int chan_num,
            IN int seg_max)
{
    ...
    /* If start is against tedge it may have been clipped */
    if (1 == istart) {
                           ...
                           IN segment_details_t* seg_details)
{
    ...
    for (iside = 0; iside < 4; iside++) {
        ...
        if ((tr_i < 0) || (tr_i > num_grid_columns)) {
            continue;
        }

        if ((tr_j < 0) || (tr_j > num_grid_rows)) {
            continue;
        }
                     ...
                     INOUT vector_t** * indices)
{
    ...
    indices[type] = (vector_t**) my_malloc(sizeof(vector_t*) * num_chans);

    for (chan = 0; chan < num_chans; ++chan) {
        indices[type][chan] =
            (vector_t*) my_malloc(sizeof(vector_t) * chan_len);
...
alloc_and_load_rr_node_indices(IN int nodes_per_chan,
    ...
    indices = (vector_t***) my_malloc(sizeof(vector_t**) * NUM_RR_TYPES);
    indices[IPIN] = (vector_t**) my_malloc(sizeof(vector_t*) * (num_grid_columns + 2));
    indices[SINK] = (vector_t**) my_malloc(sizeof(vector_t*) * (num_grid_columns + 2));

    for (i = 0; i <= (num_grid_columns + 1); ++i) {
        indices[IPIN][i] =
            (vector_t*) my_malloc(sizeof(vector_t) * (num_grid_rows + 2));
        indices[SINK][i] =
            (vector_t*) my_malloc(sizeof(vector_t) * (num_grid_rows + 2));
    ...
    load_chan_rr_indices(nodes_per_chan, num_grid_columns + 1, num_grid_rows + 1, CHANX,
                         seg_details, index, indices);
    load_chan_rr_indices(nodes_per_chan, num_grid_rows + 1, num_grid_columns + 1, CHANY,
                         seg_details, index, indices);
...
void
free_rr_node_indices(IN vector_t** * rr_node_indices)
{
    int i, j, ofs;
    ...
    for (i = 0; i < (num_grid_rows + 1); ++i) {
        for (j = 0; j < (num_grid_columns + 1); ++j) {
 ...
 *                                                                          *
 * An tedge is added from this segment to a y-segment if:                    *
                    ...
                    IN vector_t** *switch_block_conn)
{
    ...
    side_types_t from_side_a, from_side_b, to_side;
    assert(from_seg ==
...
static int
get_bidir_track_to_chan_seg(IN vector_t conn_tracks,
                            IN vector_t** * rr_node_indices,
                            IN int to_chan,
                            IN int to_seg,
                            IN int to_sb,
                            ...
                            INOUT struct s_linked_edge** edge_list)
{
    ...
    for (iconn = 0; iconn < conn_tracks.nelem; ++iconn) {
        ...
        for (i = 0; i < 2; ++i) {
            ...
            /* Add the tedge to the list */
            *edge_list = insert_in_edge_list(*edge_list,
                                             to_node,
                                             switch_types[i]);
            /* Mark the tedge as now done */
            rr_edge_done[to_node] = TRUE;
            ++num_conn;
        }
    }
                             ...
                             INOUT struct s_linked_edge** edge_list)
{
    ...
    *Fs_clipped = FALSE;
    /* SBs go from (0, 0) to (num_grid_columns, num_grid_rows) */
    is_corner = ((sb_x < 1) || (sb_x >= num_grid_columns)) && ((sb_y < 1) || (sb_y >= num_grid_rows));
    is_fringe = (FALSE == is_corner) && ((sb_x < 1) || (sb_y < 1)
                                         || (sb_x >= num_grid_columns) || (sb_y >= num_grid_rows));
...
short*****
alloc_sblock_pattern_lookup(IN int num_grid_columns,
                            IN int num_grid_rows,
                            IN int nodes_per_chan)
{
    ...
    items = 1;
    items *= (num_grid_columns + 1);
    i_list = (short*****)my_malloc(sizeof(short****) * items);
    items *= (num_grid_rows + 1);
    ...
    result = i_list;
    i_list += (num_grid_columns + 1);     /* Skip forward num_grid_columns+1 items */

    for (i = 0; i < (num_grid_columns + 1); ++i) {
        result[i] = j_list;
        j_list += (num_grid_rows + 1); /* Skip forward num_grid_rows+1 items */
                           ...
                           INOUT short**** *sblock_pattern)
{
    ...
    /* SB's have coords from (0, 0) to (num_grid_columns, num_grid_rows) */
    assert(i >= 0);
    assert(i <= num_grid_columns);
    assert(j >= 0);
    assert(j <= num_grid_rows);
    /* May 12 - 15, 2007
     *
     ...
     * fringe / chip tedge sblock which is most troublesome, as balance in each side of muxes is
     ...
     */
    /* SB's range from (0, 0) to (num_grid_columns, num_grid_rows) */
    /* First find all four sides' incoming wires */
    x_edge = ((i < 1) || (i >= num_grid_columns));
    y_edge = ((j < 1) || (j >= num_grid_rows));
    ...
    for (side = 0; side < 4; ++side) {
        ...
        chan_len = (vert ? num_grid_rows : num_grid_columns);
