...
static void add_route_tree_to_heap(t_rt_node* rt_node,
                                   int target_node,
                                   double target_criticality,
                                   double astar_fac);

static void timing_driven_expand_neighbours(struct s_heap* current,
                                            int inet,
                                            double bend_cost,
                                            double criticality_fac,
                                            int target_node,
                                            double astar_fac);

static double get_timing_driven_expected_cost(int inode,
                                             int target_node,
                                             double criticality_fac,
                                             double R_upstream);
...
boolean
try_timing_driven_route(router_opts_t router_opts,
                        double** net_slack,
                        double** net_delay,
                        vector_t** fb_opins_used_locally)
{
    ...
    for (itry = 1; itry <= router_opts.max_router_iterations; itry++) {
        ...
        if (itry == 1) {
            ...
            /* Avoid overflow for high iter counts, even if acc_cost is big */
...
void
alloc_timing_driven_route_structs(double** pin_criticality_ptr,
                                  int** sink_order_ptr,
                                  t_rt_node** * rt_node_of_sink_ptr)
{
    ...
    pin_criticality =
        (double*)my_malloc((max_pins_per_net - 1) * sizeof(double));
...
void
free_timing_driven_route_structs(double* pin_criticality,
                                 int* sink_order,
                                 t_rt_node** rt_node_of_sink)
{
...
static void
timing_driven_check_net_delays(double** net_delay)
{
